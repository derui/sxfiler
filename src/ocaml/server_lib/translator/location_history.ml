...
let to_yojson t =
  `Assoc ["records", `List (List.map Location_record.to_yojson t.records);
...
let of_yojson js =
  ...
  try
    ...
    let records = List.fold_left (fun accum record ->
        match accum with
        | Error _ -> accum
        | Ok accum -> begin match Location_record.of_yojson record with
