...
let key_of_yojson js =
  ...
  try
    ...
    Condition.of_yojson condition >>= fun condition ->
    Ok {T.key;action;condition}
  with Type_error (s, _) -> Error s

let to_yojson t =
  `Assoc ["bindings", `List (List.map key_to_yojson t.T.bindings)]

let of_yojson js =
  let open Yojson.Safe.Util in
  try
    let bindings = js |> member "bindings" |> to_list in
    let bindings = List.fold_left (fun accum key ->
...
let of_domain t =
  ...
  List.map (fun (cond, key, value) -> {
        T.condition = Condition.of_domain cond;
