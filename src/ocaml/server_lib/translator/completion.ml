...
module Item = struct
  ...
  let of_yojson (js:Yojson.Safe.json) : (t, string) result =
    try
      ...
      and value = js |> member "value" |> to_string in
      Ok {id;value}
...
module Candidate = struct
  ...
  let to_yojson t : Yojson.Safe.json =
    `Assoc ["start", `Int t.start;
            "length", `Int t.length;
            "value", Item.to_yojson t.value;]
  ...
  let to_domain t = {
    C.Candidate.start = t.start;
    length = t.length;
    value = Item.to_domain t.value;
  }

  let of_domain t = {
    start = t.C.Candidate.start;
    length = t.length;
