  ...
  let copy_file ~source ~dest ~cb =
    ...
    let rec copy_file' ic oc =
      ...
      if buf = "" then Lwt.return_unit
        ...
        copy_file' ic oc
    in
    Lwt_io.with_close_connection (fun (ic, oc) -> copy_file' ic oc) (ic, oc) ;%lwt cb ~source ~dest
  ...
  let rec copy_item ~source ~dest ~file_counter ~cb =
    ...
    | Unix.S_DIR ->
      ...
      Lwt_list.iter_s
        ...
        files
    | _ -> File_counter.add_total_count 1 file_counter ;%lwt copy_file ~source ~dest ~cb
