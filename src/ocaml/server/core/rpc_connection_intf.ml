...
module type Connection = sig
  ...
  (** [write_output t ~frame] writes a [frame] to peer on websocket connection. This function calls
      writer given with calling {!connect}. *)
  ...
  (** [connect t output_writer] connect from websocket to [t] with [output_writer]. Connection is
      saved as global. *)
  ...
  (** [default_input_handler t f] handles frame [f] with default behavior. Handling frame with
      connection provided from this module will call when user do not have any behavior what you
      want. *)
