...
let setup_command_source props _ =
  ...
  let commands =
    DL.names L.dynamic_command_registry
    |> List.map (fun action_name -> {T.Completion.Item.id = action_name; value = action_name})
  in
  let module Ctx = (val L.context) in
    ...
    ~render:(fun props ->
        ...
        [%c
          P_completer_wrapper.t
            ...
            [ [%c
              P_command_selector.t ~key:"completer"
                ~props:
                  (object%js
                    val focused = focused
