...
let activate_mode (module L : Locator.S) =
  ...
  let activation = C.Usecase.make_instance
      (module U.Activate_mode.Make(Service)) ~param:C.Types.Mode.Complete in
  Lwt.ignore_result (
    ...
    ~spec:R.(
        component_spec
          ...
          ~should_component_update:(fun this new_props _ ->
              ...
              let to_ws store = S.Workspace.Store.get @@ S.App.State.workspace @@ S.App.Store.get store in
