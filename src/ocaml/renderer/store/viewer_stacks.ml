...
module File_tree = struct
  ...
  let get t ~name = Jstable.find t.scanners Js.(string name)
                  |> Js.Optdef.to_option
  ...
  let move_index t ~name ~direction =
    ...
    let t = get t ~name >>= (fun tree ->
      ...
      >>= fun tree -> Jstable.add t.scanners Js.(string name) tree; Some t
    in
...
module State = struct
  ...
  let reduce t = function
    ...
    | C.Message.Move_cursor_to_next ->
      {file_tree = File_tree.move_index t.file_tree ~name:t.file_tree.File_tree.current_tree ~direction:`Next}
    | C.Message.Move_cursor_to_prev ->
      {file_tree = File_tree.move_index t.file_tree ~name:t.file_tree.File_tree.current_tree ~direction:`Prev}
    | _ -> t
