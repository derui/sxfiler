...
(* header of file list *)
let header = R.Component.make_stateless
    ~props:(module struct
             class type t = object
      ...
      R.component_spec
        ...
        ~component_will_receive_props:(fun this new_props ->
            ...
            ignore (
              ...
              let vl = VL.recalculate_visible_window new_props##.selectedItemIndex vl
              in
              this##setState (object%js
        ...
        (fun this ->
           ...
           let scroll_bar =
             ...
             [%c C_scroll_bar.t ~key:"scroll-bar" ~props:(object%js
                 val start = start
                 val windowSize = size
               end)]
           in
