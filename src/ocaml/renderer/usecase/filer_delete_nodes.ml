(** This module defines use case to delete nodes in filers.  *)
open Sxfiler_core
...
module Make (Service : S.Filer.S) : C.Usecase.S with type param = param' = struct
  ...
  let execute t dispatcher =
    ...
    let delete_nodes () =
      let%lwt message =
        try%lwt
          let%lwt () = Service.delete_nodes {workbench_id = t.param.workbench_id} in
    ...
    let update_filer pos =
      ...
      | Error _ ->
        ...
        Lwt.return DI.(Dispatcher.dispatch this message)
    in
    Lwt.(delete_nodes () >>= fun () -> Lwt.join [update_filer `Left; update_filer `Right])
end
