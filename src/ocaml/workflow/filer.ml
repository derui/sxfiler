...
let move input =
  ...
  | Some target -> (
      ...
      | Error _, _ | _, Error _ -> S.return { Move.events = []; result } )
...
let delete input =
  ...
  let module Common = (val common : Common_step_common.Instance) in
  let module Filer_step = (val filer_step : Common_step_filer.Instance) in
  let file_window = file_window_from_side input.Delete.filer input.side in
  ...
  | Some target -> (
      ...
      | Ok dest_file_window ->
          S.return { Delete.events = [ Updated (input.side, dest_file_window) ]; result = deleted_item }
      | Error `Not_same     -> S.return { Delete.events = []; result = deleted_item } )
...
let up_directory { Up_directory.filer; side } =
  let open S.Infix in
  let* common = S.fetch ~tag:(fun c -> `Step_common_instance c) in
  let module CS = (val common : Common_step_common.Instance) in
  let* filer = Option.to_result ~none:Up_directory.Not_initialized filer |> S.return in
  match filer with
  | Error e  -> S.return_error e
  | Ok filer -> (
      let file_window =
...
let toggle_mark { Toggle_mark.filer; side; item_id } =
  let open Result in
  let mark_item filer =
    let file_window = file_window_from_side filer side in
