...
let lwt_reporter ppf =
  ...
  let report src level ~over k msgf =
    ...
    let with_timestamp h src k ppf fmt =
      ...
      let timestamp =
        Printf.sprintf "%04d-%02d-%02dT%02d:%02d:%02d.%03.0f" (utc.Unix.tm_year + 1900) (succ utc.Unix.tm_mon)
          utc.Unix.tm_mday utc.Unix.tm_hour utc.Unix.tm_min utc.Unix.tm_sec
          (1000.0 *. (now -. floor now))
      in
      Format.kfprintf k ppf ("[%s]%a[%s] @[" ^^ fmt ^^ "@]@.") timestamp Logs.pp_header (level, h) (Logs.Src.name src)
    in
