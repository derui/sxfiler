  ...
  describe("Node Item", () => {
    describe("Size slot", () => {
      it("should print zero size", () => {
        const tree = renderer.create(wrap(<T size={bigInt(0)} />)).toJSON();

        expect(tree).toMatchSnapshot();
      });

      it("should print byte size when size less than 1024", () => {
        const tree = renderer.create(wrap(<T size={bigInt(1023)} />)).toJSON();

        expect(tree).toMatchSnapshot();
      });

      it("should print kibibyte size when size between 1KiB and 1023KiB", () => {
        const tree = renderer.create(wrap(<T size={bigInt(1024 * 2)} />)).toJSON();
