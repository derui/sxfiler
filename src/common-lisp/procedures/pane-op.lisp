...
(defun update-history (state old-pane new-pane)
  "Update active history of STATE if OLD-PANE and NEW-PANE located different directory."
  (check-type state sxfiler/state:state)
  (check-type old-pane pane)
  (check-type new-pane pane)
  (if (string/= (pane-directory old-pane) (pane-directory new-pane))
      (progn
        (setf (state-active-pane-history state)
              (let ((history (state-active-pane-history state)))
                (push-record history (pane->record (state-active-pane state)))))
        state)
      state))
 ...
 "
  ...
  (let ((active-pane (state-active-pane state)))
    (let ((new-pane (enter-focused-directory active-pane)))
      ...
      (update-history state active-pane new-pane))))
...
(defun move-focus-on-active-pane (state amount)
  ...
  (let ((active-pane (state-active-pane state)))
    (let ((new-pane (sxfiler/types/pane:move-focus active-pane amount)))
      (setf (state-active-pane state) new-pane)
      (update-history state active-pane new-pane))))
