...
let rpc_connection = [
  Alcotest_lwt.test_case "can connect and disconnect" `Quick (fun _ () ->
      ...
      let%lwt _ = Lwt.return @@ C.Connection.write_output conn
          ~frame:(Some (Websocket_cohttp_lwt.Frame.create ()))
      in
      let%lwt _ = C.Connection.disconnect conn in
      let%lwt _ =
          Lwt.return @@ C.Connection.write_output conn ~frame:(Some (Websocket_cohttp_lwt.Frame.create ()))
      in
  ...
  Alcotest_lwt.test_case "can not use disconnected connection twice" `Quick (fun _ () ->
      ...
      let%lwt _ = Lwt.return @@ Conn.write_output conn ~frame:(Some (Websocket_cohttp_lwt.Frame.create ())) in
