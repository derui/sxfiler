...
let testcases = [
  "get_node should return None if file is not found", `Quick, (fun () ->
      ...
      Alcotest.(check @@ option @@ of_pp Fmt.nop) "Not found" None (R.get_node "parent" "not_found")
    );
  ...
  Alcotest_lwt.test_case "find_by_dir from directory that contains regular files" `Quick (fun switch () ->
      let module Dummy = struct
        let getcwd () = Sys.getcwd ()
      end in
      ...
      let%lwt nodes = I.Node_repo.find_by_dir ~dir:path in
      let module N = Sxfiler_domain.Node in
      let nodes = List.map (fun v -> v.N.full_path) nodes
