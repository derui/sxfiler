module R = Sxfiler_infrastructure.Ws_connection
...
let test_set =
  [
    Alcotest_lwt.test_case "can connect and disconnect" `Quick (fun _ () ->
        ...
        let%lwt _ = Lwt.return @@ C.Connection.write_output conn ~frame:(Some (Websocket.Frame.create ())) in
    ...
    Alcotest_lwt.test_case "can not use disconnected connection twice" `Quick (fun _ () ->
        ...
        let%lwt _ = Lwt.return @@ Conn.write_output conn ~frame:(Some (Websocket.Frame.create ())) in
