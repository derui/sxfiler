...
let test_set =
  ...
  [
    Alcotest_lwt.test_case "add key binding by work flow" `Quick (fun _ () ->
        ...
        in
        let resolve_keymap () = Lwt.return empty in
        let store_keymap _ = Lwt.return_unit in
        let%lwt ret = FL.Keymap.(add_key_binding resolve_keymap store_keymap command) in
        let actual =
          empty
          |> add
               ~binding:
    ...
    Alcotest_lwt.test_case "return error when invalid key sequence by add work flow" `Quick (fun _ () ->
        ...
        let resolve_keymap () = Lwt.return empty in
        let store_keymap _ =
          Alcotest.fail "do not store key map" |> ignore;
          Lwt.return_unit
        in
        let%lwt ret = FL.Keymap.(add_key_binding resolve_keymap store_keymap command) in
    ...
    Alcotest_lwt.test_case "remove key binding by work flow" `Quick (fun _ () ->
        ...
        let resolve_keymap () = Lwt.return empty in
        let store_keymap _ = Lwt.return_unit in
        let%lwt add_result = FL.Keymap.(add_key_binding resolve_keymap store_keymap command) in
        let%lwt remove_result =
          match add_result with
          | Ok [ FL.Keymap.Added keymap ] ->
              FL.Keymap.(
                remove_key_binding (fun () -> Lwt.return keymap) store_keymap Remove_key_binding.{ key; context })
    ...
    Alcotest_lwt.test_case "return error when invalid key sequence by remove work flow" `Quick (fun _ () ->
        ...
        let%lwt ret = FL.Keymap.(remove_key_binding resolve_keymap store_keymap command) in
        let actual = Error (FL.Keymap.Invalid_key "-a") in
        let event = Alcotest.testable FL.Keymap.pp_event FL.Keymap.equal_event in
        Alcotest.(check & result (list event) & of_pp FL.Keymap.pp_error) "command" actual ret;
        Lwt.return_unit);
    Alcotest_lwt.test_case "reload whole key map from path" `Quick (fun _ () ->
        ...
        FL.Keymap.(reload load_keymap store_keymap { path = Path.of_string "/" |> Result.get_ok }) |> Lwt.ignore_result;
        Lwt.return_unit);
    Alcotest_lwt.test_case "error when can not load key map from path" `Quick (fun _ () ->
        ...
        let%lwt ret = FL.Keymap.(reload load_keymap store_keymap { path = Path.of_string "/" |> Result.get_ok }) in
        let event = Alcotest.testable FL.Keymap.pp_event FL.Keymap.equal_event in
