...
let () =
  "Filer service implementation" >::: [
    "should return filer if it created" >:- (fun () ->
        let expected = T.Filer.{
            id = "foo";
            location = "loc";
            nodes = [];
            history = T.Location_history.empty;
          } in
        let module Client = Util.Make_client(struct
            let gen id =
              ...
              R.Response.{
                empty with id = id;
                           result = Some (Js.Unsafe.coerce @@ Tr.Filer.to_js expected)
              }
          end) in
    ...
    "should return error if it already created" >:- (fun () ->
        let module Client = Util.Make_client(struct
            let gen id =
              let module R = Jsonrpc_ocaml_jsoo in
              R.Response.{
    ...
    "should raise error when get response another error" >:- (fun () ->
        let module Client = Util.Make_client(struct
            let gen id =
              let module R = Jsonrpc_ocaml_jsoo in
              R.Response.{
                empty with id = id;
                           error = Some R.Error.{
                               code = R.Types.Error_code.Internal_error;
                               data = None;
                             }
              }
          end) in
  ...
  "Filer service implementation: Get" >::: [
    "should return filer if it exists" >:- (fun () ->
        let expected = T.Filer.{
            id = "foo";
            location = "loc";
            nodes = [];
    ...
    "should return error if it do not find" >:- (fun () ->
        let module Client = Util.Make_client(struct
            let gen id =
              let module R = Jsonrpc_ocaml_jsoo in
              R.Response.{
