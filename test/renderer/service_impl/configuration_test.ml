...
let () =
  "Configuration service implementation" >::: [
    "should return configuration" >:- (fun () ->
        let module D = Sxfiler_domain in
        let expected = T.Configuration.{
            default_sort_order = D.Types.Sort_type.(to_int Date);
          } in
        let module Client = Util.Make_client(struct
            let gen id =
              ...
              R.Response.{
                empty with id = id;
                           result = Some (Js.Unsafe.coerce @@ Tr.Configuration.to_js expected)
              }
          end) in
