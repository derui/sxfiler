...
import func from 'sxfiler/renderer/components/file-list/mode-converter';

/** @test {ModeConverter} */
let wrapper = (label, f) => test(`renderer/components/FileList/ModeConverter ${label}`, f);

wrapper('should return ---------- when given NaN', (t) => {
  t.equal(func(NaN), '----------');
  t.end();
});

wrapper('should return ---------- when type of given value is not number', (t) => {
  t.equal(func('0'), '----------');
  t.equal(func(''), '----------');
  t.equal(func([]), '----------');
  t.equal(func({}), '----------');
...
wrapper('should return posix-style permission of given permission', (t) => {
  [0, 1, 2].forEach((shift) => {
...
wrapper('should return file type of directory or symlink', (t) => {
